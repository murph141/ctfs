...
class SecurityEngine():
    def __init__(self):
        self.commands = {
                ...
                "read_key":   (self.cmd_read_key,   ["keyslot"]),
    ...
    def cmd_encrypt(self, keyslot, dest, src):
        ...
        print(f'Encrypting: {plaintext}, {key}, {ciphertext}')
    ...
    def cmd_read_key(self, slot_idx):

        try: slot_idx = self._parse_slot_idx(slot_idx)
        except: return "ERROR: Invalid index"

        return self.keyslots[SLOT_SIZE*slot_idx:SLOT_SIZE*(slot_idx+1)].hex()
    ...
    def cmd_print(self):
        ...
        return f"""Printing values:
running:     {self.running}
counter:     {self.counter}
keyslots {len(self.keyslots)}:  {self.keyslots}
dataslots {len(self.dataslots)}: {self.dataslots}
...
async def handle_client_safely(reader, writer):
    ...
    try:
        await asyncio.wait_for(handle_client(reader, writer), None)
        writer.close()
